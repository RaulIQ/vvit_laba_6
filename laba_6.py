...
class Calculator(QWidget):
	def __init__(self):
		...
		self.hbox_null.addWidget(self.b_clean)
		...
		self.b_negative = QPushButton("-x", self)
		self.hbox_fourth.addWidget(self.b_negative)
		...
		self.b_clean.clicked.connect(lambda: self._button("C"))
		...
		self.b_negative.clicked.connect(lambda: self._button('-'))
	...
	def _operation(self, op):
		try:
			...
			self.input.setText('Введите вначале число.')
		...
		try:
			...
			if self.op == "/":
				try:
					...
					self.input.setText('Не дели на ноль')
		except ValueError:
			self.input.setText('Введите вначале число.')
